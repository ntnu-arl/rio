...
class EvaluateGroundTruth:
    def __init__(self, rosbag_name, rosbag_dir, ground_truth_csv, pose_topic, odom_topic, export_directory, dataset_type):
        ...
        if not (self.dataset_type == FINAL_POS):
            gt_csv_path = rosbag_dir + ground_truth_csv
            self.ground_truth = GroundTruth(gt_csv_path, dataset_type)
        ...
        self.sub_odometry = rospy.Subscriber(odom_topic, Odometry, self.callback_odometry, queue_size=1000000)
    ...
    def export(self):
        ...
        if len(self.export_directory) > 0:
            ...
            if self.dataset_type == FINAL_POS:
                csv_vio = self.export_directory + "/" + self.rosbag_name[:-4] + ".csv"
                data_vio = np.vstack(self.pose_data)
                np.savetxt(csv_vio, data_vio, delimiter=' ', fmt='%.3f')
            else:
                ...
                np.savetxt(csv_gt, data_gt, delimiter=' ')  # , fmt='%.6f')
                np.savetxt(csv_vio, data_vio, delimiter=' ')  # , fmt='%.6f')
...
if __name__ == "__main__":
    ...
    pose_topic = rospy.get_param('~pose_topic', "pose_not_used")
    odom_topic = rospy.get_param('~odom_topic', "odom_no_used")
    export_directory = rospy.get_param('~export_directory', "")
    dataset_type = str(rospy.get_param('~dataset_type', ""))

    evaluator = EvaluateGroundTruth(rosbag_name, rosbag_dir, ground_truth_csv, pose_topic, odom_topic, export_directory, dataset_type)
